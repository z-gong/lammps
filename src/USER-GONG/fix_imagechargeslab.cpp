...
FixImageChargeSlab::FixImageChargeSlab(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg) {
  scalar_flag = 0;
  vector_flag = 0;
  size_vector = 4;
  global_freq = 1;
  ...
  if (narg != 9) error->all(FLERR, "Illegal fix imagechargeslab command");

  igroup2 = group->find(arg[3]);
  if (igroup2 == -1)
    error->all(FLERR, "Fix imagechargeslab image group ID does not exist");
  if (group->count(igroup) * 2 != group->count(igroup2))
    error->all(FLERR, "Fix imagechargeslab number of image atoms does not equal to two times of electrolyte atoms");
  ...
  if (igroup_cathode == -1)
    error->all(FLERR, "Fix imagechargeslab cathode group ID does not exist");
  ...
  if (igroup_anode == -1)
    error->all(FLERR, "Fix imagechargeslab anode group ID does not exist");
  groupbit_anode = group->bitmask[igroup_anode];

  z_anode = force->numeric(FLERR, arg[8]);
  z_span = z_anode - z_cathode;
  if (z_span <= 0 || z_cathode < domain->zprd / 3 + domain->boxlo[2] || z_anode > domain->boxhi[2] - domain->zprd / 3)
    error->all(FLERR, "Fix imagechargeslab the positions of electrodes are not valid");

  n_cathode = group->count(igroup_cathode);
  n_anode = group->count(igroup_anode);

  memory->create(img_parent, atom->natoms + 1, "fix_imagechargeslab::img_parent");
  memory->create(xyz, atom->natoms + 1, 3, "fix_imagechargeslab::xyz");
  memory->create(xyz_local, atom->natoms + 1, 3, "fix_imagechargeslab::xyz_local");
  memory->create(xyz_img_tmp, 3, "fix_imagechargeslab::xyz_img_tmp");
  memory->create(charge, atom->natoms + 1, "fix_imagechargeslab::charge");
  memory->create(charge_local, atom->natoms + 1, "fix_imagechargeslab::charge_local");
}

/* ---------------------------------------------------------------------- */

FixImageChargeSlab::~FixImageChargeSlab() {
...
int FixImageChargeSlab::setmask() {
  int mask = 0;
  mask |= PRE_EXCHANGE;
  mask |= PRE_FORCE;
  return mask;
...
void FixImageChargeSlab::init() {
  if (!(domain->xperiodic && domain->yperiodic && !domain->zperiodic))
    error->all(FLERR, "Fix imagechargeslab should be used with periodic xy and non-periodic z");

  double eps0 = 8.854187E-12; // F/m
  double e0 = 1.602176E-19; // coulomb
  double A_m = 1.0E-10; // m/Angstrom
  // assume box size does not change in x and y direction
  q_voltage = V * domain->xprd * domain->yprd / z_span * eps0 / e0 * A_m;
  if (comm->me == 0) {
    if (screen) {
      fprintf(screen, "Image charge constant voltage method of Petersen\n");
      fprintf(screen, "  assuming box not change during simulation. units are real\n");
      fprintf(screen, "  charge %.4f spread on %d cathode atoms and %d anode atoms\n",
              q_voltage, n_cathode, n_anode);
    }
    if (logfile) {
      fprintf(logfile, "Image charge constant voltage method of Petersen\n");
      fprintf(logfile, "  assuming box not change during simulation. units are real\n");
      fprintf(logfile, "  charge %.4f spread on %d cathode atoms and %d anode atoms\n",
              q_voltage, n_cathode, n_anode);
    }
  }
...
void FixImageChargeSlab::pre_exchange(int /*vflag*/) {
  update_img_positions();
  update_electrode_charges();
}
...
void FixImageChargeSlab::pre_force(int /*vflag*/) {
  update_img_positions();
  update_electrode_charges();
}

/* ---------------------------------------------------------------------- */
void FixImageChargeSlab::build_img_parents() {
  ...
  for (int i = 1; i < atom->natoms + 1; i++) {
    // set the default of image_parent to 0. since the atom tags starts from 1. negative value is used for anode images
    img_parent[i] = 0;
  ...
  for (int i = 0; i < tag_parents.size(); i++) {
    ...
    // images on anode. set the tag of parent to negative so can be distinguished
    img_parent[tag_imgs[i] + tag_parents.size()] = -tag_parents[i];
  }
}

/* ---------------------------------------------------------------------- */

void FixImageChargeSlab::assign_img_charges() {
  int tag, tag_parent;
  int *mask = atom->mask;
  double *q = atom->q;
...
void FixImageChargeSlab::update_img_positions() {
  int tag, tag_parent;
  int *mask = atom->mask;
  double **x = atom->x;
...
void FixImageChargeSlab::update_electrode_charges() {
  int *mask = atom->mask;
  double **x = atom->x;
  double *q = atom->q;
  ...
  for (int ii = 0; ii < atom->nlocal; ii++) {
    if (mask[ii] & groupbit) {
      q_electrodes_local[0] += q[ii] * (x[ii][2] - z_cathode) / z_span;
      q_electrodes_local[1] += q[ii] * (z_anode - x[ii][2]) / z_span;
    }
  }
