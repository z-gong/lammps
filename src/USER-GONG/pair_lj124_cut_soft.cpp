...
PairLJ124CutSoft::PairLJ124CutSoft(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 0;
  writedata = 0;
}
...
PairLJ124CutSoft::~PairLJ124CutSoft()
{
  if (allocated) {
...
void PairLJ124CutSoft::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,r4,forcemie,factor_mie;
  double denmie,denmie2;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        denmie2 = denmie * denmie;

        forcemie = mie1[itype][jtype]* 4 * rsq *
            (3*denmie2*denmie2-denmie2)* mie2[itype][jtype];
        ...
        if (eflag) {
          evdwl = mie1[itype][jtype] * (denmie2 * denmie - denmie) -
            offset[itype][jtype];
          evdwl *= factor_mie;
        }
...
void PairLJ124CutSoft::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJ124CutSoft::settings(int narg, char **arg)
{
...
void PairLJ124CutSoft::coeff(int narg, char **arg)
{
  if (narg < 5 || narg > 6)
...
double PairLJ124CutSoft::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  static const double Cmie = 2.598076211353316;
  ...
  if (offset_flag) {
    ...
    offset[i][j] = mie1[i][j] * (denmie * denmie * denmie - denmie);
  } else offset[i][j] = 0.0;
  ...
  if (tail_flag) {
    ...
    double siggamR = pow(sigma[i][j],12);
    double rcgamA = cut[i][j];
    double rcgamR = pow(cut[i][j],9);
    etail_ij = Cmie*2.0*MY_PI*all[0]*all[1]*epsilon[i][j]*
      (siggamR/(9*rcgamR)-siggamA/rcgamA);
    ptail_ij = Cmie*2.0*MY_PI*all[0]*all[1]*epsilon[i][j]/9.0*
      (4.0*siggamR/rcgamR-12.0*siggamA/rcgamA);
  }
...
void PairLJ124CutSoft::write_restart(FILE *fp)
{
...
void PairLJ124CutSoft::read_restart(FILE *fp)
{
...
void PairLJ124CutSoft::write_restart_settings(FILE *fp)
{
...
void PairLJ124CutSoft::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
double PairLJ124CutSoft::single(int i, int j, int itype, int jtype, double rsq,
                           ...
                           double &fforce)
{
  ...
  forcemie = mie1[itype][jtype]*4.0*rsq*
      (3.0*denmie2*denmie2 - denmie2)* mie2[itype][jtype];
  fforce = factor_mie*forcemie;

  phimie = mie1[itype][jtype] * (denmie2 * denmie - denmie) -
    offset[itype][jtype];
...
void *PairLJ124CutSoft::extract(const char *str, int &dim)
{
  dim = 2;
