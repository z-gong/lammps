...
PairGaussCutGZ::PairGaussCutGZ(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 0;
}
...
void PairGaussCutGZ::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        ugauss = hgauss[itype][jtype]*exp(-0.5*rexp*rexp);
...
void PairGaussCutGZ::settings(int narg, char **arg)
{
...
void PairGaussCutGZ::coeff(int narg, char **arg)
{
...
double PairGaussCutGZ::init_one(int i, int j)
{
  if (setflag[i][j] == 0) error->all(FLERR,"All pair coeffs are not set");

  if (offset_flag) {
    double rexp = (cut[i][j]-rmh[i][j])/sigmah[i][j];
    offset[i][j] = hgauss[i][j] * exp(-0.5*rexp*rexp);
  } else offset[i][j] = 0.0;
...
void PairGaussCutGZ::write_restart(FILE *fp)
{
...
void PairGaussCutGZ::read_restart(FILE *fp)
{
...
void PairGaussCutGZ::write_restart_settings(FILE *fp)
{
...
void PairGaussCutGZ::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
double PairGaussCutGZ::single(int i, int j, int itype, int jtype, double rsq,
...
/* ---------------------------------------------------------------------- */
double PairGaussCutGZ::memory_usage()
{
