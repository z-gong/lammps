...
FixPPM::FixPPM(LAMMPS *lmp, int narg, char **arg) :
        Fix(lmp, narg, arg),
...
void FixPPM::min_setup(int vflag) {
    post_force(vflag);
}

/* ---------------------------------------------------------------------- */

void FixPPM::post_force(int vflag) {
    double **x    = atom->x;
    double **f    = atom->f;
    int    *type  = atom->type;
...
void FixPPM::post_force_respa(int vflag, int ilevel, int iloop) {
...
void FixPPM::min_post_force(int vflag) {
    post_force(vflag);
}
...
double FixPPM::compute_vector(int n) {
    return 0;
}
