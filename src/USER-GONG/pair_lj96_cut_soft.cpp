...
PairLJ96CutSoft::PairLJ96CutSoft(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 0;
  writedata = 0;
}
...
PairLJ96CutSoft::~PairLJ96CutSoft()
{
  if (allocated) {
...
void PairLJ96CutSoft::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,r4,r6,forcemie,factor_mie;
  double denmie;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        r4 = rsq*rsq;
        r6 = r4*rsq;
        denmie = 1.0/(mie3[itype][jtype] + r6*mie2[itype][jtype]);

        forcemie = mie1[itype][jtype]* 6.0 * r4 *
...
void PairLJ96CutSoft::settings(int narg, char **arg)
{
...
void PairLJ96CutSoft::coeff(int narg, char **arg)
{
  if (narg < 5 || narg > 6)
...
double PairLJ96CutSoft::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  if (tail_flag) {
    ...
    double siggamA = pow(sigma[i][j],6);
    double siggamR = pow(sigma[i][j],9);
    double rcgamA = pow(cut[i][j],3);
    double rcgamR = pow(cut[i][j],6);
    etail_ij = Cmie*2.0*MY_PI*all[0]*all[1]*epsilon[i][j]*
      (siggamR/(6.0*rcgamR)-siggamA/(3.0*rcgamA));
    ptail_ij = Cmie*2.0*MY_PI*all[0]*all[1]*epsilon[i][j]/3.0*
...
void PairLJ96CutSoft::write_restart(FILE *fp)
{
...
void PairLJ96CutSoft::read_restart(FILE *fp)
{
...
void PairLJ96CutSoft::write_restart_settings(FILE *fp)
{
...
void PairLJ96CutSoft::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
double PairLJ96CutSoft::single(int i, int j, int itype, int jtype, double rsq,
                           double factor_coul, double factor_mie,
                           double &fforce)
{
  double r4,r6,forcemie,phimie;
  double denmie;
...
void *PairLJ96CutSoft::extract(const char *str, int &dim)
{
  dim = 2;
