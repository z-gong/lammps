...
PairGaussCutFep::PairGaussCutFep(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 0;
}
...
PairGaussCutFep::~PairGaussCutFep()
{
  if (allocated) {
...
void PairGaussCutFep::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        ugauss = lambda[itype][jtype]*hgauss[itype][jtype]*exp(-0.5*rexp*rexp);
...
void PairGaussCutFep::settings(int narg, char **arg)
{
...
void PairGaussCutFep::coeff(int narg, char **arg)
{
...
double PairGaussCutFep::init_one(int i, int j)
{
  if (setflag[i][j] == 0) error->all(FLERR,"All pair coeffs are not set");

  if (offset_flag) {
    double rexp = (cut[i][j]-rmh[i][j])/sigmah[i][j];
    offset[i][j] = lambda[i][j] * hgauss[i][j] * exp(-0.5*rexp*rexp);
  } else offset[i][j] = 0.0;
...
void PairGaussCutFep::write_restart(FILE *fp)
{
...
void PairGaussCutFep::read_restart(FILE *fp)
{
...
void PairGaussCutFep::write_restart_settings(FILE *fp)
{
...
void PairGaussCutFep::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
double PairGaussCutFep::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairGaussCutFep::extract(const char *str, int &dim)
{
  dim = 2;
